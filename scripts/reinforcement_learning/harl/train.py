...
def main(env_cfg: ManagerBasedRLEnvCfg | DirectRLEnvCfg | DirectMARLEnvCfg, agent_cfg: dict):
    ...
    # synchronize HARL and Isaac Lab devices with the AppLauncher device flag
    # - if user passes `--device cpu`, run both physics and RL on CPU
    # - otherwise, keep default (typically CUDA)
    device_str = args.get("device", None)
    if isinstance(device_str, str):
        # update Isaac Lab simulation device
        if hasattr(env_cfg, "sim") and hasattr(env_cfg.sim, "device"):
            env_cfg.sim.device = device_str

        # update HARL runner/device settings
        # HARL uses a simple cuda: bool flag in its config
        if "device" in algo_args and isinstance(algo_args["device"], dict):
            algo_args["device"]["cuda"] = "cuda" in device_str
